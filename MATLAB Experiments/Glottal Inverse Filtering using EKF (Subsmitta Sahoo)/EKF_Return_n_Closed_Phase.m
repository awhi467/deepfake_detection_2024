% MATLAB function EKF_Return_n_Closed_Phase.m is the EKF estimation algorithm
% in the return phase and closed phase.
%
% Description:
...
function [est_V,X1,Theta1,P11,ug,states]=EKF_Return_n_Closed_Phase(V,X0,Theta0,P00,S0,Q0,R0,L,Fs)
N=2*L+2+L+3;

Tau=1/Fs;
Theta10=Theta0;
[P10, X10]=Find_P_2ndPart(X0,Theta0,P00,Tau,S0,Q0,L);
C_Theta=[zeros(1,L-1) 1+Theta0(L+1) zeros(1,L+2)];
G1=P10(:,L)*(1+Theta0(L+1))/(P10(L,L)*(1+Theta0(L+1))^2+R0);
P11=(eye(N-1)-G1*[C_Theta zeros(1,L+2)])*P10;
...
function [P10, X10]=Find_P_2ndPart(X0,Theta0,P00,Tau,S0,Q0,L)

[A_Theta, A_Theta_X]=Find_A_Theta_X_2ndPart(X0,Theta0,Tau,L);
del_A_Theta=Find_partialderivtv_2ndPart(X0,Theta0, Tau,L);

tmp=[A_Theta del_A_Theta; zeros(L+2,2*L+2) eye(L+2)];
P10=tmp*P00*tmp'+[Q0,zeros(2*L+2,L+2);zeros(L+2,2*L+2),S0];
X10=A_Theta_X;
end

function [A_Theta, A_Theta_X]=Find_A_Theta_X_2ndPart(X,Theta,Tau,L)
...
A_Theta_X(2*L+1)=X(2*L+1)+Tau*X(2*L+2);
A_Theta_X(2*L+2)=-Theta(L+2)*Tau*X(2*L+2)+X(2*L+2);
...
A_Theta(2*L+1,:)=[zeros(1,2*L) 1 Tau];
A_Theta(2*L+2,:)=[zeros(1,2*L) 0 1-Tau*Theta(L+2)];
end

function del_A_Theta=Find_partialderivtv_2ndPart(X,Theta, Tau,L)
...
del_A_Theta(2*L+2,L+2)=-Tau*X(2*L+2);
end